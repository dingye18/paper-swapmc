...
\begin{center}
  \includegraphics[height = 3.5cm]{figures/toc.pdf}
\end{center}
...
Recent advances in force field development, including OPLS4~\cite{lu2021opls4}, CGenFF~\cite{vanommeslaeghe2012automationI, vanommeslaeghe2012automationII}, GAFF2~\cite{he2022recent}, \textblue{and OpenFF~\cite{wang2024open,Boothroyd2023}}, have substantially improved molecular representation accuracy~\cite{Hahn_2024}, 
...
The GCMC method has demonstrated substantial improvements in FEP accuracy, reducing prediction errors from $<$3 kcal/mol to $<$1.5 kcal/mol for systems with water displacement~\cite{ross2020enhancing}.
...
However, these methods face different limitations: GCMC and GCNCMC require system-dependent chemical potential calibration; 
...
Implementation of SwapMC method within the Uni-FEP framework using the OpenMM~\cite{Eastman2023} simulation engine provides seamless integration with existing FEP workflows without requiring specialized Monte Carlo packages or external tools. 
...
\newline
To validate the effectiveness of SwapMC, we conducted comprehensive tests on established benchmark sets and compared its performance with GCMC and standard MD approaches~\cite{ross2020enhancing}. 
...
The buffer distance (0.3~nm) ensures inclusion of the first hydration shell beyond the ligand van der Waals surface. 
...
\newline
The SwapMC algorithm consists of these steps:
\begin{enumerate}
  ...
  \textblue{
  The 18,000 conformations are re-generated independently for each swap attempt, ensuring the insertion probability $g^{(ins)}_{S_j}$ remains uniform over the destination region as required by Eq.~\ref{eq:transition_probability}.
  }
...
For states $S_i$ and $S_j$ that differ only in a water molecule position:
...
The $n!$ terms cancel out when computing probability ratios, simplifying the acceptance criterion derivation.
And $U\left(\mathbf{x}_1, \ldots, \mathbf{x}_i,\ldots, \mathbf{x}_n\right)$, $U\left(\mathbf{x}_1, \ldots, \mathbf{x}_j,\ldots, \mathbf{x}_n\right)$ denote the potential energy of the states $S_i, S_j$.
...
\newline
The transition probability matrix element $G_{S_i \to S_j}$ quantifies the likelihood of transition from state $S_i$ to state $S_j$. 
...
The SwapMC method is implemented within the Uni-FEP framework built upon OpenMM~\cite{Eastman2023}, 
...
Long-range electrostatics in interaction energies are computed using the Reaction-Field method ($\epsilon = 80$) to approximate Particle Mesh Ewald (PME)~\cite{petersen1995accuracy} interactions while enabling fully GPU-local computation without global FFT operations. \\
\newline 
Water molecule selection from the source region employs the Gumbel-Max Trick for GPU compatibility~\cite{gumbel1954statistical,huijben2022review}.
\textblue{
...
\newline
\textred{
Our validation strategy comprises three stages: 
(1) demonstrating that SwapMC accelerates water equilibration via direct sampling analysis; 
(2) evaluating RBFE accuracy by comparison with GCMC benchmarks, 
including robustness assessments across different initial water configurations, 
as well as comparisons for systems without water sampling challenges to confirm 
that SwapMC does not impair performance when water sampling is not a limiting factor; 
and 
(3) extending validation to ABFE calculations.
}
...
\newline
All molecular dynamics simulations were performed using the Uni-FEP framework, built upon OpenMM~\cite{Eastman2023}. 
Both relative (RBFE) and absolute (ABFE) binding free energy calculations employed identical base simulation conditions: NPT ensemble at 298.15 \textit{K} and 1 bar, with a 4 \textit{fs} timestep.
The TIP3P~\cite{Jorgensen1983} water model was used throughout, with proteins described by the ff14SB~\cite{Maier2015} force field and ligands parameterized using GAFF2~\cite{he2020fast}, 
employing AM1-BCC partial charges computed via antechamber. \\
\newline
RBFE calculations employed 5~\textit{ns} production simulations per $\lambda$-window, with $\lambda$-spacing dependent on transformation type: 
24 $\lambda$-windows for charge-change and core-hopping transformations, and 16 $\lambda$-windows for other transformation types. 
ABFE calculations employed longer production simulations (20~\textit{ns} per $\lambda$-window) to enable thorough equilibration of protein conformational changes during alchemical decoupling, 
with 32 $\lambda$-windows for neutral ligands and 48 $\lambda$-windows for charged ligands. \\
\newline
\textblue{
...
SwapMC moves were performed every 1000 MD steps (4 ps) in both RBFE and ABFE simulations, with 300 uniformly distributed insertion sites sampled within the spherical cavity region per attempt. \\
\newline
\textblue{
Beutler soft-core potential~\cite{Beutler_1994} were employed for van der Waals interactions to 
...
To illustrate SwapMC's water sampling capabilities, we examined the alchemical transformation of Ligand 9 to Ligand 7 within the HSP90 binding site (Fig.~\ref{fig:hsp90} A), 
...
\newline
Without SwapMC, contact water numbers remain suppressed throughout the transformation, 
...
In contrast, SwapMC-enabled simulations rapidly repopulate the cavity, 
achieving near-equilibrium contact water numbers and maintaining stable populations thereafter. \\
\newline
The differential water occupancy between ligand endpoints reflects chemical intuition: 
Ligand 7 accommodates 4.87 $\pm$ 1.57 contact waters compared to 4.18 $\pm$ 1.00 for Ligand 9 from simulations without SwapMC.
With SwapMC, the number of contact waters are increased substantially (12.79 $\pm$ 1.39 for Ligand 7, 9.0 $\pm$ 2.28 for Ligand 9). 
The increment of contact waters is consistent with the transformation of Ligand 9 to Ligand 7 with larger open space in cavity of the protein pocket. 
...
For the Ligand 9~$\to$~7 transformation, SwapMC reduced the error in $\Delta \Delta G$ from 8.171 $\pm$ 0.037 kcal/mol (no SwapMC) to 2.702 $\pm$ 0.036 kcal/mol (with SwapMC) compared to the experimental value of -1.49 kcal/mol. 
...
\noindent
Water sampling was substantially improved by the SwapMC method, 
enabling reliable application of RBFE calculations to systems where water displacement plays a critical role in binding. 
Using the water-displacement benchmark introduced by Ross et al.~\cite{ross2020enhancing}, 
we evaluated SwapMC on the same eight protein–ligand systems and 94 ligand transformations to ensure direct comparability. \\
\newline
For each system, four RBFE calculation protocols were performed: with and without SwapMC, and with and without inclusion of crystallographic cavity waters (CW) in the initial structures (CW defined as crystallographic waters within 6~$\AA$~of ligand heavy atoms).
\textblue{
...
with standard MD used for production simulations (Equi. SwapMC Prod. NoSwapMC in Table~\ref{tbl:rbfe_watersets}).
...
All simulations employed an identical FEP protocol and force field setup, as described in the Methods, to isolate the effect of the sampling strategy. \\
\newline
The resulting RBFE statistics are summarized in Table~\ref{tbl:rbfe_watersets}. 
Incorporation of SwapMC consistently reduced the RMSE with respect to experimental binding free energies and generally increased $R^2$ values across the majority of systems, 
demonstrating a marked improvement in thermodynamic accuracy relative to standard FEP. 
Moreover, RBFE results obtained with SwapMC were largely insensitive to whether crystallographic cavity waters were retained or removed in the initial structures, 
indicating that SwapMC rapidly equilibrated the cavity hydration state and thus confered robustness with respect to input water placement. 
\textblue{
...
Finally, the accuracy achieved with SwapMC was comparable to that reported for GCMC-based water sampling on the same benchmark set, 
supporting the conclusion that SwapMC provides an effective and practically attractive alternative to GCMC for treating water sampling in RBFE calculations. \\
...
\begin{table}
  ...
  The $\Delta\Delta G$ RMSE and $R^2$ values are computed using experimental binding free energies. 
  The unit of $\Delta\Delta G$ RMSE is \textit{kcal/mol}, with $R^2$ presented in parentheses. 
  The optimal results are highlighted in \textbf{bold}.}
  \label{tbl:rbfe_watersets}
  \scriptsize
  \begin{tabular}{l|llllll}
    \hline
    System                                  & No CW.     & No CW.        & With CW.    & With CW.     &  Equi. SwapMC      & GCMC by  \\
    $\Delta \Delta G$ RMSE ($R^2$)          & No SwapMC  & With SwapMC   & No SwapMC   & With SwapMC  &  Prod. NoSwapMC    & Gregory et al.~\cite{ross2020enhancing} \\
    \hline
    Brd4(1)                      & 1.34 (0.67) & 0.87 (0.82) & 1.07 (0.71) & \textbf{0.64 (0.93)}     &  0.96 (0.84)       & 1.74 (0.01) \\
    Chk1                         & 2.99 (0.02) & 1.67 (0.43) & 2.64 (0.10) & 1.70 (0.47)              &  2.00 (0.25)       & \textbf{0.78 (0.80)} \\
    HSP90\_Kung                  & 3.85 (0.11) & 1.51 (0.59) & 3.90 (0.16) & \textbf{1.50} (0.50)     &  2.16 (0.32)       & 2.14 (\textbf{0.62}) \\
    HSP90\_Woodhead              & 2.27 (0.83) & 0.61 (0.98) & 2.25 (0.84) & 0.65 (0.97)              &  1.01 (0.89)       & \textbf{0.26} (\textbf{1.00}) \\
    Scytalone                    & 2.25 (0.73) & 1.45 (0.84) & 1.90 (0.80) & \textbf{1.39 (0.85)}     &  1.23 (0.89)       & 1.86 (0.77) \\
    Taf1(2)                      & 2.01 (0.08) & 0.70 (0.51) & 0.76 (0.34) & 0.66 (0.51)              &  0.77 (0.48)       & \textbf{0.66 (0.78)} \\
    Thrombin                     & 1.37 (0.34) & 1.14 (0.64) & 1.20 (0.46) & \textbf{1.13 (0.66)}     &  0.92 (0.63)       & 1.15 (0.49) \\
    Urokinase                    & 0.74 (0.62) & 0.91 (0.36) & 0.81 (0.52) & 0.94 (0.32)              &  0.87 (0.42)       & \textbf{0.70 (0.74)} \\
    \hline
  \end{tabular}
\end{table}
...
where enabling SwapMC produces substantial corrections to $\Delta\Delta G$ predictions (see Table~\ref{tbl:rbfe_watersets}). 
...
To extend our evaluation beyond relative binding free energy calculations, we assessed SwapMC performance on absolute binding free energy (ABFE) calculations, where the sampling challenge is more stringent. 
ABFE requires complete decoupling of the ligand from the protein cavity through a series of alchemical transformations, 
a process fundamentally dependent on rapid and accurate sampling of cavity water reorganization. 
The disruption of protein-ligand interactions necessitates substantial rearrangement of the hydration network, 
and inadequate water equilibration during this reorganization systematically biases the computed binding free energy~\cite{Liu2025}. 
Recent studies have demonstrated that enhanced water sampling substantially improves ABFE convergence and accuracy, motivating systematic evaluation of SwapMC on this more challenging FEP variant. \\
\newline
ABFE calculations were performed on the JACS benchmark set, comprising eight diverse protein targets (BACE, CDK2, JNK1, MCL1, P38, PTP1B, Thrombin, TYK2) 
with a total of 67 ligands distributed across varying degrees of binding site accessibility and water displacement magnitude. 
This diverse set enables assessment of SwapMC utility across a broad range of binding site properties. 
All ABFE calculations followed the identical FEP protocol and thermodynamic cycle as described in the Methods section, with SwapMC active throughout both the bound-state and solution-phase decoupling stages. \\
\newline
Table~\ref{tbl:abfe_jacs} presents ABFE results across the eight protein targets in the JACS benchmark set. 
\textblue{
...
SwapMC improved performance in six of eight systems, reducing $\Delta G$ RMSE relative to standard MD.
The range of $\Delta G$ RMSE decreased from 1.67 $\sim$ 8.78 kcal/mol (no SwapMC) to 0.77 $\sim$ 4.94 kcal/mol (with SwapMC).
However, the magnitude of improvement shows substantial system-dependent variation, with particularly notable gains for systems where cavity water reorganization is the dominant sampling bottleneck. \\
\newline
The most substantial improvements occurred for PTP1B (RMSE 8.78 $\to$ 4.94 kcal/mol, 44\% reduction), TYK2 (2.51 $\to$ 1.35 kcal/mol, 46\% reduction), and CDK2 (3.99 $\to$ 2.04 kcal/mol, 49\% reduction), 
where SwapMC enabled more accurate treatment of water displacement during ligand decoupling. 
Concurrent $R^2$ improvements for these systems (CDK2: 0.09 $\to$ 0.55, TYK2: 0.40 $\to$ 0.47) 
\textblue{and Kendall's $\tau$ (CDK2: 0.20 $\to$ 0.38, TYK2: 0.38 $\to$ 0.50)} 
...
\newline
In contrast, two systems showed minimal or negligible benefit from SwapMC. P38 showed essentially unchanged accuracy (RMSE 3.22 $\to$ 3.25 kcal/mol), 
with $R^2$ improvement (0.31 $\to$ 0.46) suggesting SwapMC slightly enhanced sampling efficiency without substantially reducing errors—consistent with this system's relatively hydrophobic ATP-binding site where water reorganization is limited. % CHECK the literatures for this!
\textblue{
...
%Thrombin showed a small accuracy degradation (RMSE 1.67 $\to$ 1.82 kcal/mol), though $R^2$ improved substantially (0.35 $\to$ 0.72), 
%indicating improved correlation despite slightly elevated RMSE. 
%This apparent contradiction reflects elimination of systematic bias: without SwapMC, Thrombin predictions fortuitously exhibited high $R^2$ despite substantial systematic errors, 
%whereas SwapMC corrected this bias at the cost of modest RMSE increase for some individual ligands. \\
\newline
The system-dependent variation in SwapMC benefit correlates with the magnitude of water displacement required during decoupling. 
PTP1B's buried active site and kinase targets (P38, CDK2, JNK1, TYK2) show the largest improvements, consistent with these systems requiring substantial water network reorganization that benefits from enhanced sampling.
In contrast, Thrombin's solvent-accessible binding site and BACE's moderate solvent exposure show smaller effects, 
reflecting that water exchange kinetics are less rate-limiting for these more exposed cavities. 
\textblue{
...
%These observations validate the mechanistic premise that SwapMC specifically addresses water-sampling-limited ABFE scenarios, 
%improving accuracy when water reorganization is the bottleneck but providing minimal benefit when other factors (force field accuracy, protein conformational sampling) dominate the prediction error.
...
\begin{table}[ht]
  \caption{
    ABFE Results for JACS set with and without SwapMC.
    Shifted dG RMSE is computed after subtracting the per-target mean signed error 
    to remove the systematic contribution from incomplete apo-to-holo conformational sampling. 
    Kendall's $\tau$ assesses rank-ordering accuracy.}
  \label{tbl:abfe_jacs}
  \centering
  \footnotesize
  ...
  \begin{tabular}{l|cccc|cccc}
    \hline
    \multirow{2}{*}{System}
      & \multicolumn{4}{c|}{No SwapMC}
      & \multicolumn{4}{c}{With SwapMC} \\
    \cline{2-9}
      & \makecell{dG RMSE} & \makecell{$R^2$}
      & \makecell{Shifted\\dG RMSE} & \makecell{Kendall's\\$\tau$}
      & \makecell{dG RMSE} & \makecell{$R^2$}
      & \makecell{Shifted\\dG RMSE} & \makecell{Kendall's\\$\tau$} \\
    \hline
    BACE     & 3.27 & 0.25 & 1.56 & 0.37 & \textbf{2.68} & 0.20 & 1.10 & 0.26 \\
    CDK2     & 3.99 & 0.09 & 1.16 & 0.20 & \textbf{2.04} & 0.55 & 0.96 & 0.38 \\
    JNK1     & 2.01 & 0.48 & 0.72 & 0.52 & \textbf{0.77} & 0.63 & 0.72 & 0.54 \\
    MCL1     & 2.05 & 0.34 & 1.82 & 0.44 & \textbf{1.67} & 0.36 & 1.41 & 0.32 \\
    P38      & 3.22 & 0.31 & 1.08 & 0.41 & 3.25          & 0.46 & 0.96 & 0.51 \\
    PTP1B    & 8.78 & 0.29 & 1.82 & 0.38 & \textbf{4.94} & 0.33 & 1.51 & 0.38 \\
    Thrombin & 1.67 & 0.35 & 1.31 & 0.38 & 1.82          & 0.72 & 0.57 & 0.60 \\
    TYK2     & 2.51 & 0.40 & 1.14 & 0.38 & \textbf{1.35} & 0.47 & 1.07 & 0.50 \\
    \hline
  \end{tabular}
\end{table}
...
\subsection{Simulation of Pocket Collapse in the Absence of Cavity Waters}
In addition to improving water sampling during FEP calculations, 
SwapMC also mitigates the risk of binding site collapse during equilibration when initial structures lack cavity waters. 
Ge et al.~\cite{Ge2022} reported a pocket collapse event in the BPTI system (PDB ID: 5PTI), 
in which complete decoupling of a buried water molecule created a transient cavity that induced an inward displacement of the GLY12 backbone oxygen, 
leading to a collapsed binding site, bimodal work distributions, 
and poor phase-space overlap in non-equilibrium switching (NES) simulations. \\
\newline
To investigate whether SwapMC can prevent such collapse events, 
we performed 10 ns plain MD simulations of the BPTI system starting from a structure with the target cavity water molecule removed. 
A 2 $fs$ timestep was employed, with temperature and pressure maintained at 300 K and 1 atm, respectively; 
force field settings and long-range interaction treatments were consistent with those described in the FEP calculations. 
With the default SwapMC swap interval of 1000 MD steps (2~$ps$), 
no successful insertion moves were observed throughout the 10 ns simulation, 
indicating that the cavity was not rehydrated under standard settings. \\
\newline
Upon reducing the swap interval to 100 MD steps (0.2 ps), successful insertion moves were achieved, 
and the target water molecule was reinserted into the cavity region within 10 ns across all three independent simulations. 
The minimum distance between the inserted water oxygen and the cavity center 
— defined as the geometric centroid of the backbone nitrogen atoms of Cys14 and Cys38 and the backbone oxygen atoms of Cys38 and Thr11 
— was tracked throughout each simulation and is presented in Fig.~\ref{fig:5pti_pocket_collapse}. \\
\newline
These results demonstrate that SwapMC can effectively prevent pocket collapse by maintaining cavity hydration during equilibration, 
provided that the swap interval is appropriately tuned. 
The failure of the default interval to achieve reinsertion highlights the system-dependence of optimal SwapMC parameters, 
particularly in cases involving deeply buried, high-barrier cavities. 
The relatively low acceptance rate for insertion moves in this system further suggests that additional parameter optimization or algorithmic refinements 
may be warranted to enhance sampling efficiency when pocket collapse presents a significant risk.

\begin{figure}
  \includegraphics[width=0.9\textwidth]{figures/5pti_pocket_collapse.png}
  \caption{
Pocket collapse prevention in the BPTI system (PDB ID: 5PTI). 
(Left) Initial structure with the target cavity water molecule highlighted in red, 
positioned at the geometric centroid of the backbone nitrogen atoms of Cys14 and Cys38 and the backbone oxygen atoms of Cys38 and Thr11. 
(Right) Minimum distance from water oxygen to the cavity center during three 
independent 10 ns simulations with SwapMC swap interval of 100 MD steps (0.2 ps).
  }
  \label{fig:5pti_pocket_collapse}
\end{figure}

\section{Conclusion and Discussion}
We have presented Swap Monte Carlo (SwapMC), a novel GPU-accelerated method designed to address water sampling challenges in protein-ligand binding free energy calculations. 
SwapMC operates within the NPT ensemble and avoids the chemical potential calibration required by Grand Canonical Monte Carlo methods, 
making it accessible for routine implementation in existing FEP workflows. 
Through comprehensive benchmarking on 94 RBFE transformations across eight protein systems and 67 ABFE calculations across eight additional targets, 
we have demonstrated that SwapMC substantially improves both water sampling efficiency and thermodynamic accuracy for systems where cavity water reorganization significantly impacts binding affinity. \\
\newline
For RBFE calculations on water-displacement-sensitive systems from Ross et al.'s benchmark set~\cite{ross2020enhancing}, 
SwapMC achieved accuracy comparable to GCMC while providing critical practical advantages. 
The HSP90 Ligand 9~$\to$~Ligand 7~transformation exemplifies SwapMC's capabilities: starting from structures with cavity waters removed, 
SwapMC rapidly equilibrated the water distribution and improved the RBFE prediction by 4.59 kcal/mol compared to standard MD. 
Across the full benchmark set, SwapMC improved accuracy for the majority of systems, with particularly notable gains for targets with buried binding sites (Brd4, HSP90 Kung, HSP90 Woodhead). 
Critically, SwapMC demonstrated robustness to initial water configuration—RBFE accuracy remained consistent whether crystallographic waters were included or removed from input structures, 
contrasting with standard MD's sensitivity to initial water placement. 
This robustness eliminates a time-consuming and expertise-dependent step in FEP setup workflows. 
For ABFE calculations, SwapMC improved accuracy for six of eight tested systems, with substantial benefits for targets where water reorganization during ligand decoupling dominates the sampling challenge.\\
\newline
The SwapMC implementation achieves computational efficiency through GPU-parallelized evaluation of water insertion candidates, 
inheritance of OpenMM's neighbor list infrastructure, and use of the Gumbel-Max trick for biased water selection. 
The method is compatible with the Uni-FEP framework and maintains simulation performance comparable to standard MD, making it practical for production drug discovery applications. 
Several promising extensions warrant investigation: adapting SwapMC to sample ion distributions in nucleic acid systems, enabling co-solvent sampling in binding sites, and extending to ligand fragment or co-factor sampling in multi-component binding scenarios. 
These extensions could broaden SwapMC's applicability to diverse biomolecular systems where sampling of small molecules beyond water represents a computational bottleneck~\cite{Cezar2020,goel2021rapid}. 
We anticipate that SwapMC's combination of accuracy, robustness, and ease of implementation will facilitate broader adoption of water-aware FEP methods in structure-based drug discovery.
...
This work was supported by National Key R\&D program of China (2022YFA1004302).
