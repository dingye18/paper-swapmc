...
According to the Metropolis criteria, in simulations, the acceptance ratio for transforming state $S_i$ to state $S_j$, denoted as $A_{S_i,S_j}$, is defined as:
...
Thus, the equilibrium probabilities $P_{S_i}$ and $P_{S_j}$ follow a Boltzmann distribution:
...
\begin{equation}\label{potential_energy_interaction}
...
\end{equation}
where $g_{S_i}^{(\text{del})}$ and $g_{S_j}^{(\text{ins})}$ represent the respective marginal probabilities for molecule removal from state $S_i$ and insertion into state $S_j$. 
This probabilistic decomposition is algebraically demonstrated in the derivation steps.
...
as their respective source regions differ between states $S_i$ and $S_j$.
...
Through substitution of the components in Eq.~\ref{eq:acceptance_ratio} with the ensemble averages defined in Eq.~\ref{}, 
we obtained the generalized acceptance criterion:
\begin{equation}
