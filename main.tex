...
While with SwapMC, the number of contact waters are increased obviously (12.79 $\pm$ 1.39 for Ligand 7, 9.0 $\pm$ 2.28 for Ligand 9). 
...
indicating that SwapMC rapidly equilibrates the cavity hydration state and thus confers robustness with respect to input water placement. 
Finally, the accuracy achieved with SwapMC is comparable to that reported for GCMC-based water sampling on the same benchmark set, 
...
SwapMC demonstrated performance gains for six of eight systems, with six targets showing reduced $\Delta G$ RMSE compared to standard MD. 
...
The HSP90 Ligand 9~$\to$~transformation exemplifies SwapMC's capabilities: starting from structures with cavity waters removed, 
