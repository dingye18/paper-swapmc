...
FEP method employed molecular dynamics (MD) simulation to sample the conformation space of the system.
...
and the sampling over the conformation space~\cite{15rex, wang2011replica, hritz2008hamiltonian} of the system.
Free energy perturbation (FEP) is increasingly utilized in structure-based drug design to optimize ligand selectivity and affinity.\\
\newline
FEP calculations can be broadly categorized into two types: relative binding free energy (RBFE) calculations and absolute binding free energy (ABFE) calculations.
The RBFE calculations are consisted of a series of alchemical transformations, 
where the ligand is transformed from one state to another in the binding site of the protein.
And ABFE calculations are performed by decoupling the ligand from the protein cavity.
...
which can lead to significant changes in the water distribution of the binding pocket.
Sampling water molecules buried in binding sites necessitates a comprehensive exploration of the conformational space~\cite{zhou2009theory, cozzini2004free,li2007water}, 
...
GCMC allowed the insertion and deletion move of water molecules in the binding site during the MD simulations.
Water molecules moves that are accepted or rejected based on the Metropolis criteria.
The detailed balance condition is satisfied by the acceptance ratio derived from the chemical potential and the interaction energy of the water molecules.
Insertion and deletion moves enable efficient sampling of the position and number of water molecules in the binding site,
which improves the accuracy of binding free energy calculations.
However, the acceptance ratio of GCMC is often low due to the high density of water molecules in the binding site.
An extended way of GCMC is the Nonequilibrium Candidate Monte Carlo (GCNCMC)~\cite{Bergazin2020,Ge2022,melling2023enhanced,Deng2024},
that combines nonequilibrium dynamics with Monte Carlo moves to enhance the acceptance ratio of water insertion and deletion moves.
The relaxtion of the system during the nonequilibrium dynamics improves the sampling efficiency of water molecules in the binding site.
